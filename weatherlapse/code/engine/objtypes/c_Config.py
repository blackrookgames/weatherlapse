...
import code.engine.xml2 as _xml2

from code.engine.num import\
    Parse as _Parse
...
class Config(_xml2.XmlLoadable, _xml2.XmlSavable):
    ...
    def __init__(self):
        ...
        self.__deletecache:bool = self.__DEF_DELETECACHE
    ...
    def _load_from_xml(self, element:_ET.Element):
        ...
        if x_apikey is not None:
            ...
            self.__apikey = ""
        # useragent
        x_useragent = _xml2.XmlUtil.find_element(element, "useragent")
        if x_useragent is not None:
            self.__useragent = "" if (x_useragent.text is None) else x_useragent.text
        else:
            self.__useragent = ""
        ...
        # deletecache
        x_deletecache = _xml2.XmlUtil.find_element(element, "deletecache")
        if x_deletecache is not None:
            self.__deletecache = _xml2.XmlUtil.parse_element_text(x_deletecache, _Parse.try_bool)
        else:
            self.__deletecache = self.__DEF_DELETECACHE
    ...
    def _save_to_xml(self, element:_ET.Element):
        ...
        # useragent
        x_useragent = _xml2.XmlUtil.create_element(tag = "useragent", text = self.__useragent)
        element.append(x_useragent)
        # #region (leave the hash or else vscode will think it's the start of a region of code)
        ...
        # deletecache
        x_deletecache = _xml2.XmlUtil.create_element(tag = "deletecache", text = str(self.__deletecache))
        element.append(x_deletecache)
    ...
    @property
    def useragent(self):
        """ User Agent (ex: 'WeatherApp/1.0 (johndoe@email.com)')"""
    ...
    def deletecache(self):
        """ Whether or not the cache should be deleted when exiting the program """
        return self.__deletecache
    @deletecache.setter
    def deletecache(self, value:bool):
        self.__deletecache = value
