...
def _process(appdir:str, iswindows:bool, outdir:str, queue:_mp.Queue):
    global _LAYERS, _TILE_DIM, _IMAGE_POINT
    # What time is it?
    dt = _dt.datetime.now()
    ...
    try:
        ...
        rawimage_params = { 'appid': config.apikey }
        ...
        for _tile_y in range(tiles_rows):
            for _tile_x in range(tiles_cols):
                ...
                # Fetch
                _res = _rq.get(_url, params = rawimage_params)
                if _res.status_code != 200: raise Exception(f"OpenWeatherMap Status {_res.status_code}")
                # Add to image
                with _Image.open(_io.BytesIO(_res.content)).convert('RGBA') as _img:
        ...
        extra_path = appinfo.cache_world_bg_path(config.region, False)
        with open(extra_path, 'rb') as _f: output.add(_misc.NAME_LAND_O, _f.read())
        log.write("Saving fill\n")
        extra_path = appinfo.cache_world_bg_path(config.region, True)
        with open(extra_path, 'rb') as _f: output.add(_misc.NAME_LAND_F, _f.read())
        # Save meta
        ...
        output.add(_misc.NAME_META, meta_bytes.getvalue())
        # Save
